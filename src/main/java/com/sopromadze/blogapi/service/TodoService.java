...
public class TodoService {
    ...
    public PagedResponse<Todo> getAllTodos(UserPrincipal currentUser, int page, int size){
        ...
        if (todos.getNumberOfElements() == 0){
            return new PagedResponse<>(Collections.emptyList(), todos.getNumber(), todos.getSize(), todos.getTotalElements(), todos.getTotalPages(), todos.isLast());
        }

        return new PagedResponse<>(todos.getContent(), todos.getNumber(), todos.getSize(), todos.getTotalElements(), todos.getTotalPages(), todos.isLast());
    }
